...
## [2026-07-14] schema | 2000-shuffle rerun + H3 first visual-onset (null) + Jul-23 package

Three work streams, all landed and committed on `dev-hammad` the same day.

**Paper-grade H1 rerun (NHR@FAU TinyGPU, jobs 1744669/1744899)**: 140/140 cells (28 pieces x 5 levels) at 2000 shuffles, merged to `data/processed/tier3/_latency_grid_2000.csv` (force-added). Headline drops unchanged vs the 100-shuffle grid: onset synchrony clean→Zoom 56.5% Dagstuhl / 65.1% ESMUC / 75.1% ChoralSynth / 70.7% corpus; envelope E flat (Dagstuhl -0.4%). Two infrastructure incidents, both root-caused and fixed in the committed sbatch: (1) SLURM tasks inherited a PATH without `~/.local/bin`, so all 140 tasks of the first array died at `uv: command not found` (exit 127); fix: PATH export inside the script. (2) Concurrent `uv run` invocations sync the venv against the core spec and stripped the extras mid-run (65 tasks died with `ModuleNotFoundError: librosa` while already-loaded tasks kept computing); fix: `uv run --no-sync`, the venv is read-only for tasks and sync happens once on the login node. Cluster policies encoded in the script: partition `work`, `--gres=gpu:1` mandatory, no `--mem-per-cpu` on GPU jobs.

**H3 first visual-onset attempt (`scripts/h3_visual_onset.py`, TDD, 5 new tests)**: the Jun-25 promised analysis ran on the 18 pose-usable Tier-1 videos: pose-derived motion (z-scored |Δ| of head_sway/trunk_lean/shoulder_rise, single tracked stream, first-minute window, 10 Hz grid) vs the MP4 audio onset envelope (imageio-ffmpeg decode, new pinned dep in wp1-audio), max-lag r within ±2 s, 1000 circular-shift nulls per video with the same lag search per draw. **Result: informative null.** 17/18 analyzable (VJ3TLIFHBGw's first 90 s are digitally silent, flagged `usable_audio=False`, not hidden); 1/17 significant at p < 0.05, which is chance; median max-lag r 0.068. Estimator validity is CI-tested on synthetic coupled signals (recovers known lags, rejects independent streams), so the null speaks about the data configuration: ensemble motion of one tracked body does not couple to a mixed audio track. H3's ΔR² claim stays data-blocked; the paired-corpus requirement is now demonstrated, not assumed. Outputs: `data/processed/tier1/_h3_visual_onset.csv` + `data/figures/h3_visual_onset.png` (both force-added).

**Jul-23 final package**: `jul23_deck.md` (14 slides + reproducibility backup, every number source-annotated), `jul23_script.md` (Hammad slides 1-5, Zuraiz slides 6-14 + 60 s live demo choreography with fallback), `jul23_qa_prep.md` (15 entries incl. the 56.5-vs-57 rounding answer and the cluster-incident answer), `scripts/generate_jul23_pptx.py` → `output/jul23_final_presentation.pptx` (15 slides, reload-verified). Professors confirmed the live-demo format at Status VI. PROJECT_GUIDE §11 Claims 1/3 + roadmap updated; TEAM_BRIEF §3 refreshed. Suite 44/44, ruff clean on all new/changed files. Numbering note: the H2 null mean is exactly 0.1385 in `_h2_centralization.csv`; all materials cite it half-up as 0.139 (a first draft of the Jul-23 package briefly used 0.138 via numpy's half-to-even rounding; corrected before delivery).
