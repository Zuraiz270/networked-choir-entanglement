...
**SNA-OSN-M Summer 2026 · Uni Bamberg · Last updated 2026-07-09**
...
2. An internal seminar report, 10-20 pages in IEEE/LNCS style (per Status Meeting III: internal report, not a peer-reviewed paper). Zuraiz lead author.
...
## 3. Where we are today (updated 2026-07-14)
...
| **Tier 2 data** (Dagstuhl + ESMUC + ChoralSynth)         | **Dagstuhl on disk 2026-05-17** (5.1 GB, MD5 `82b95faa…` verified vs Zenodo). **ESMUC + ChoralSynth downloaded + MD5-verified vs Zenodo 2026-06-15** (ESMUC `zenodo.5848990` CC BY 4.0, 2.3 GB, per-singer WAV; ChoralSynth `zenodo.10137883` CC BY-SA 4.0, 96 MB, per-voice MP3, 20 pieces). On-inspection corrections: ESMUC is **7 song codes** (DG/DH1/DH2/SC1-3/WU) × FT/IS/SE settings, not "3 pieces"; 48 multitrack groups (32 full-ensemble). Provenance at `data/raw/_dataset_inventory.md` + `_dataset_checksums.csv`. |
| **Feature code** (audio, video, network, dashboard)      | **All 4 WPs shipped Sprint-2 milestones.** WP1: SATB A(t) coupling on Dagstuhl. WP2: MediaPipe pose extraction on a Tier-1 SoundJack video (79.5% detection). WP3: Granger influence graph (11/12 SATB edges significant). WP4: dashboard wireframe + alchemical diagram drafted. **Sprint-3 Phase A (2026-05-22):** WP1 scaled to all 25 Dagstuhl musical takes (LI + TP), 130 newly-extracted singer parquets + 288 pairwise A(t) couplings; corpus summary at `data/processed/dagstuhl/_summary.csv`. **Sprint-3 Phase B (2026-05-22):** WP3 scaled to 5 pieces × 2 methods (standard Granger + COP-GC, Zanin 2024 P-22); per-piece GEXFs + network metrics at `data/processed/dagstuhl/_network_metrics.csv`; 2x3 grid figure at `data/figures/wp3_influence_graphs_5pieces.png`. 18/18 tests. **Sprint-3 Phase C (2026-05-22):** WP2 pose extraction batched across 10 Tier-1 videos stratified by NMP regime; 5/10 pass the 50% detection floor (consistent with the Status-Meeting-III "try and iterate" decision); summary at `data/processed/tier1/_pose_summary.csv`; v2 V(t) figure at `data/figures/wp2_visual_features_v2.png` (ZKthfLPWBCQ, 98.5% detection). **Sprint-3 Phase D (2026-05-22):** WP4 dashboard scaffold (React 18.3 + Vite 5.3 + TypeScript strict + D3 7.9 + Plotly 2.33 + FastAPI 0.111), 4-panel layout fetches mock JSON from FastAPI and renders end-to-end; screenshot at `data/figures/wp4_dashboard_scaffold.png`. E(t) integration module at `src/choir_entanglement/entanglement.py` with circular-shift null at the composite level; LI_QuartetA_Take02 demo: mean E = 0.744, null mean 0.573 ± 0.008, p_null = 0.0000; figure at `data/figures/et_timeline_LI_QA_Take02.png`. 23/23 tests. **Sprint-3 Phase E (2026-05-22):** E(t) run on all 5 WP3 Dagstuhl pieces with 200-shuffle null. All 5 beat null at p < 0.005; piece-level clustering LI 0.74-0.80 vs TP 0.57-0.68. Corpus table at `data/processed/dagstuhl/_et_corpus.csv`; cross-piece scatter at `data/figures/et_corpus_comparison.png`; Sprint-3 narrative + four honest limitations at `sprint3_results.md` (deck source material). **Sprint-3 Phase F (2026-05-22):** Jun-11 Status Meeting IV deliverables at `jun11_deck.md` (10 slides, Hassan presenting, strength-first), `jun11_script.md` (9-min spoken script + appendix), `jun11_qa_prep.md` (4 categories + ESMUC prepared answer), `output/jun11_status_meeting_iv.pptx` (rendered via `scripts/generate_jun11_pptx.py`). **Sprint-4 (2026-06-15/22):** ESMUC + ChoralSynth added via a unified dataset-adapter (`datasets.py`), MD5-verified, extracted. **Tier-3 latency injection (H1, the headline):** constant delay showed no E(t) effect (lag-tolerant envelope confound), pivoted to measured-SD jitter, then added zero-lag **onset synchrony** which recovers the signal: attack-timing synchrony falls **57% (Dagstuhl) / 66% (ESMUC) / 76% (ChoralSynth)** clean→Zoom across the full **28-piece, 3-dataset** corpus (envelope E(t) flat throughout). Figure `data/figures/tier3_corpus_summary.png`. **Dashboard alpha** on real data (`dashboard/app.py`: real E(t) + influence graph + pose overlay; screenshot `data/figures/wp4_dashboard_realdata.png`). **WP2** pose on all 29 Tier-1 videos (18/29 pass detection). WP3 Hacker flagship SVG rendered. 38/38 tests. Jun-25 deck/script/Q&A/PPTX ready (`jun25_*`). |
...
**Sprint 3 complete (2026-05-22)**: 6 of 6 deliverables shipped, including 2 stretch pull-forward items 23 days ahead of the Jun-14 brief target. 15 atomic commits on `main`. Headline result: E(t) integration tested on all 5 WP3 Dagstuhl pieces, all 5 beat the 200-shuffle circular-shift null at p < 0.005. WP1 batch-scaled to 25 takes. WP2 batch-processed 10 Tier-1 videos (5/10 pass detection floor per the "try and iterate" decision). WP3 added COP-GC method, full 5-piece corpus tabulated. WP4 React+Vite+FastAPI scaffold renders end-to-end against mock JSON. Status Meeting IV (Jun 11) deck + script + Q&A + PPTX ready at `jun11_deck.md` / `jun11_script.md` / `jun11_qa_prep.md` / `output/jun11_status_meeting_iv.pptx`; Hassan presented. See `sprint3_results.md` for the 1-page narrative.

**Sprint 4 complete (2026-06-22)**: the H1 latency hypothesis got its first real test, and it holds. Added ESMUC + ChoralSynth (both MD5-verified) behind one dataset adapter, then ran Tier-3 latency injection across **28 pieces in 3 datasets**. Key finding, reported honestly as a journey: a constant delay does nothing to E(t) (our envelope coupling is lag-tolerant), but realistic **jitter** degrades **attack-timing synchrony** by **57% / 66% / 76%** (Dagstuhl / ESMUC / ChoralSynth) from clean to Zoom-class, while the loudness envelope stays flat. So latency breaks *when* singers land notes, not *how loud* they are. The dashboard now runs on real data. **Status Meeting V (Jun 25): Zuraiz + Kumaran presenting**; deck/script/Q&A/PPTX ready (`jun25_*`, `output/jun25_status_meeting_v.pptx`).
...
**Status VI day (2026-07-09)**: H2's "leader dominance" now has its operational definition on the slide itself (out-degree centralization, Gini of out-degree, vs 1000 density-matched random graphs; corpus mean 0.154 vs null 0.139), directly answering Prof. Hacker's Status-V question. Branch audit passed: all deck numbers verified against `_latency_grid.csv` / `_h2_centralization.csv`, tests 39/39. One tracked follow-up for report stage: stored E(t) CSVs predate the onset-folding change and need an envelope-only flag or regeneration before reruns are cited.

**Final package day (2026-07-14)**: three things landed in one push. First, the paper-grade H1 rerun finished on the Erlangen cluster (140 of 140 cells at 2000 shuffles): the headline drops are unchanged at 56.5% / 65.1% / 75.1%, which is the best outcome, the numbers survived a 20x stricter null. The grid now lives at `_latency_grid_2000.csv` and the cluster script is committed in a form that works out of the box. Second, the long-promised H3 first attempt ran on the 18 pose-usable videos and came back null: 1 of 17 analyzable videos significant, which is what chance predicts (one video was excluded because its first 90 seconds are silent). The measurement itself is validated by tests, so the honest conclusion is that visual entanglement needs per-singer audio and video together, and that requirement is now demonstrated rather than assumed. Third, the complete Jul-23 package is committed: `jul23_deck.md` (14 slides + backup), `jul23_script.md` (Hammad slides 1-5, Zuraiz results + 60-second live demo), `jul23_qa_prep.md`, and `output/jul23_final_presentation.pptx`. Remaining before Jul 23: rehearsal on the presentation laptop, including the demo path and the fallback slide. Suite at 44 tests.
...
| 2026-06-14 ✓ (done 05-22)  | E(t) end-to-end on full Dagstuhl WP3 corpus + null model running (5/5 pieces beat null at p < 0.005); pulled forward 23 days |
...
**Sprint-2 status (2026-05-17)**: Granger module + influence-graph builder shipped in `src/choir_entanglement/network/{granger,influence_graph}.py`, with circular-shift null (200 shuffles). Pairwise Granger on Dagstuhl SATB Quartet A Take 02 RMS envelopes: 11/12 directed edges significant at p_null < 0.05, graph density 0.92, most-central voice reported as Soprano (eigenvector centrality 0.53; later found to be a near-tie with Alto, see Sprint-3 note). First draft of Hacker's flagship figure: `data/figures/wp3_influence_graph.png`. Smoke tests 4/4 pass.

**Sprint-3 status (2026-05-22)**: WP3 scaled to 5 Dagstuhl pieces × 2 methods. **COP-GC variant implemented** (Zanin 2024 ordinal-pattern transform, P-22) as `method="cop_gc"` in `granger.py`; the long-promised Decision-4 deliverable is now live. Standard Granger Sprint-2 reference reproduces on edges + density (11/12 edges, density 0.917); the "most central voice" label is a near-tie at 0.53 between Soprano and Alto that resolves by node ordering (batch run reports Alto), so we treat edges + density as the regression check and the centrality label as tie-sensitive. Method divergence is largest on TP_FullChoir (standard 42/56 vs COP-GC 25/56). 10 Gephi-compatible GEXFs + flat metrics at `data/processed/dagstuhl/_network_metrics.csv`; 2x3 grid figure (Hacker flagship v2) at `data/figures/wp3_influence_graphs_5pieces.png`. Tests 18/18 (+3 COP-GC). E(t) on full WP3 corpus also shipped (5/5 pieces beat null at p < 0.005).
