...
**Latest (2026-07-16)**: final H2 was regenerated from the 2000-shift grid with rounding-safe plus-one empirical p-values. Corpus Gini is 0.162 vs null 0.155; Dagstuhl 1/5, ESMUC 1/3, and ChoralSynth 0/20 pieces are significant. Earlier Status-VI values are superseded. Final report, Jul-23 deck, and 48-test suite are complete.
...
- **Code** (uv-only repo; scaffold 2026-04-25, WP-scoped deps, pinned `uv.lock`, CI, pre-commit; Docker dropped as over-engineered). All four WP modules under `src/choir_entanglement/`. Built over three sprints:
  ...
  - **Sprint 3 (May 22)**, scaled: WP1 to all 25 Dagstuhl takes; WP3 to 5 pieces × 2 methods (standard + COP-GC, Zanin 2024); WP2 to 10 Tier-1 videos; WP4 React + Vite + FastAPI scaffold; E(t) integration module + 200-shuffle null; full-corpus E(t) on 5 pieces (all beat null, p < 0.005).
  - **Sprint 4 (Jun 15-22)**, cross-dataset + the H1 test:
    - Dataset-adapter layer (`datasets.py` + `scripts/wp1_corpus_batch.py`) ingests ESMUC + ChoralSynth.
    - **Tier-3 latency injection** (`latency.py`, `scripts/tier3_latency_grid.py`): the journey was constant delay (no effect, lag-tolerant confound) → measured-SD jitter → **zero-lag onset synchrony** (`audio/coupling.py`), which recovers the H1 signal above.
    - Dashboard alpha on **real data** (`dashboard/app.py`, screenshot `data/figures/wp4_dashboard_realdata.png`); WP2 pose on **all 29** Tier-1 videos (18 pass 50% detection); WP3 Hacker flagship SVG.
  - **Tests 38/38**; ruff + mypy strict clean.

- **Data** (provenance in `data/raw/_dataset_inventory.md` + `_dataset_checksums.csv`):
  - **Tier-1** (YouTube, video only): 29 videos SHA-256 hashed (`data/tier1_corpus_manifest.csv`); all 29 pose-processed, 18 pass the 50% detection floor.
  - **Tier-2** (multitrack, audio + network): Dagstuhl (MD5-verified, May 17) + ESMUC + ChoralSynth (both downloaded + MD5-verified vs Zenodo, Jun 15). Per-singer feature parquets for all pieces; influence-graph GEXFs for the 5 WP3 Dagstuhl pieces.
  - **Tier-3** (latency injection): `data/processed/tier3/_latency_grid.csv` (140 cells, 28 pieces × 5 regimes).
  - **Virtual Mirror**: SC Chat Analyzer outputs at `data/sc chat analyzer/`.
...
2. **An internal seminar report.** 10-20 pages in IEEE/LNCS style (per Status Meeting III the scope is an internal report, not a peer-reviewed paper). Every claim backed by a DOI or primary citation. Must include our reproducibility appendix. Lead author: Zuraiz.
...
- **Claim 1 (H1).** Higher latency lowers coordination. Metric: zero-lag onset synchrony (and E(t) sub-scores). Status (2026-07-16): **supported in the timing channel** by paired clean-to-Zoom comparisons: onset synchrony drops 56.5% (Dagstuhl) / 65.1% (ESMUC) / 75.1% (ChoralSynth), 70.7% corpus-wide, and all 28/28 pieces decrease (exact one-sided sign-test p = 3.73 x 10^-9). Source: `data/processed/tier3/_h1_paired_test.csv`. The 2000 circular shifts in `_latency_grid_2000.csv` test envelope and influence-network cells, not the deterministic onset values. Caveat: latency is injected into pre-recorded audio, not recorded live.
- **Claim 2 (H2), redefined 2026-06-22 and corrected 2026-07-16.** Choir influence networks may carry leadership structure. Metric: **out-degree centralization (Gini of node out-degree) vs a density-matched Erdős-Rényi null** (0 = democratic, toward 1 = one leader). Status: **limited support in real human choirs** (Dagstuhl 1/5, ESMUC 1/3 pieces significant; ChoralSynth 0/20; mean Gini 0.162 vs null 0.155). The final table uses the 2000-shift grid and rounding-safe plus-one empirical p-values. The original latency-driven form is **data-blocked** because injected delay cannot create behavioural adaptation. Test: `scripts/h2_centralization_test.py`.
- **Claim 3 (H3).** Honest-Signals visual features (body sway plus breathing) add meaningfully (ΔR² ≥ 0.10) to E(t) beyond audio alone. Status (2026-07-14): the ΔR² claim remains **data-blocked** (no corpus piece has per-singer audio and video together), and the planned first attempt has now **run and returned an informative null**: pairing pose-derived motion with the audio onset envelope on the 18 pose-usable Tier-1 videos (per-video pose windows of 44.2-71.8 s, mostly about 60-72 s; max-lag r within ±2 s; 1000 circular-shift nulls per video) gives 1/17 analyzable videos significant (= chance; one video excluded for a digitally silent first 90 s), median max-lag r 0.068. The estimator recovers known lags on synthetic coupled signals (tested in CI), so the null is about the data configuration: ensemble motion of a single tracked stream does not couple to a mixed audio track. The paired-corpus requirement is now demonstrated, not assumed. Source: `data/processed/tier1/_h3_visual_onset.csv`, `scripts/h3_visual_onset.py`.
...
| **Jun 11** ✓ (done 05-22 partial) | WP3 Granger scaled to 5 Dagstuhl pieces + COP-GC variant                                              | Sprint-3 Phase B. Standard Granger + COP-GC (Zanin 2024 ordinal-pattern transform) on 5 pieces (LI QuartetA/B Take/01-02, LI FullChoir Take01, TP QuartetA/FullChoir Take01). 10 GEXFs + `_network_metrics.csv` + 2x3 grid figure at `data/figures/wp3_influence_graphs_5pieces.png`. Tests 18/18. |
...
| **Jun 14** ✓ (done 05-22 5-piece corpus) | E(t) on full Dagstuhl WP3 corpus + Sprint-3 results narrative              | Sprint-3 Phase E. All 5 WP3 pieces beat 200-shuffle null at p < 0.005; piece-level clustering LI 0.74-0.80 vs TP 0.57-0.68. `_et_corpus.csv` + cross-piece scatter `data/figures/et_corpus_comparison.png` + 1-page `sprint3_results.md` source-of-truth for the Jun-11 deck. Pulled forward 23 days from Jun-14 brief target. |
| **Jun 11** ✓ (deck ready 05-22) | **Virtual status #4 deck + script + Q&A bank rendered**            | Sprint-3 Phase F. `jun11_deck.md` (10 slides, Hassan presenting, strength-first), `jun11_script.md` (9-min spoken script), `jun11_qa_prep.md` (Hacker/Gloor/General/Hard categories), `output/jun11_status_meeting_iv.pptx` (rendered via `scripts/generate_jun11_pptx.py`). Meeting itself: Thu 14:00 CET. |
...
| **Jun 21** ✓ (done 06-21) | Dashboard alpha version                                                                             | WP4 web UI shows video + pose overlay + influence graph + E(t) timeline on **real data** (screenshot `data/figures/wp4_dashboard_realdata.png`) |
| **Jun 25**           | **Virtual status #5** (Zuraiz + Kumaran presenting)                                                      | Deck/script/Q&A ready (`jun25_*`); headline = H1 latency result on attack-timing synchrony across 3 datasets |
| **Jun 30** ✓ (mostly done 06-22) | Full pipeline on Tier 1 + Tier 2 + Tier 3                                                       | Tier-1: all 29 videos pose-processed. Tier-2: 28 pieces across 3 datasets. Tier-3: 140-cell latency grid. (Paper-scale 2000-shuffle rerun completed 07-14.) |
| **Jul 7**            | Paper draft v1                                                                                           | First complete draft circulated                                                                                                     |
| **Jul 9, 14:00-16:00 CET** ✓ done | **Virtual status #6**                                                                              | Last check-in before final. Professors confirmed a live dashboard demo for Jul 23.                                                  |
| **Jul 14** ✓ done    | 2000-shuffle grid + H3 first visual-onset + Jul-23 package                                               | Paper-grade H1 rerun on NHR@FAU (job 1744899, 140/140 cells; headline drops unchanged at 56.5/65.1/75.1%). H3 first attempt returns an informative null (17/18 analyzable, 1/17 significant ≈ chance, median r 0.068; `_h3_visual_onset.csv` + figure). Jul-23 deck follows the official 15-20 slide structure with 17 main slides plus 2 backups, a 10-12 minute script, Q&A bank, project-structure diagram, and regenerated `output/jul23_final_presentation.pptx`. |
| **Jul 23**           | **Final 20-minute presentation**                                                                   | Deck/script/Q&A/pptx ready since 07-14 (Hammad slides 1-5, Zuraiz results + 60 s live demo); rehearsal on the presentation laptop pending                                                                                                                      |
...
**What it is.** Purpose-built research datasets where each singer was recorded on a separate microphone. Primary: **Dagstuhl ChoirSet** (Rosenzweig et al., TISMIR 2020, DOI `10.5334/tismir.48`). Also: **ESMUC Choir Dataset** (Cuesta & Gómez, Zenodo DOI `10.5281/zenodo.5848990`, CC BY 4.0, **downloaded + MD5-verified 2026-06-15**; on inspection: 7 song codes DG/DH1/DH2/SC1-3/WU × FT/IS/SE settings, 48 multitrack groups / 32 full-ensemble, per-singer WAV, not the "3 pieces" the paper implies), **ChoralSynth** (synthetic SATB, Narang et al. 2023, Zenodo DOI `10.5281/zenodo.10137883`, CC BY-SA 4.0, **downloaded + MD5-verified 2026-06-15**; 20 pieces, per-voice MP3, 3-8 voices each), **Cairns 2024 York PhD thesis** (NMP comparator data), **DUST Dataset** (packet-loss traces — access to verify by Apr 24). Full verified provenance: `data/raw/_dataset_inventory.md`.
...
| 3 | Zanin 2024 (P-22) Augmenting Granger Causality through continuous ordinal patterns, CNSNS 128:107606 | L2    | 2024 | HIGH | MED  | ACCEPTED (implemented Sprint-3 Phase B 2026-05-22; `method="cop_gc"` in `granger.py`; year corrected 2026-06-11 after web verification) |
