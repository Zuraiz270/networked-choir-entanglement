...
- **Claim 2 (H2), redefined 2026-06-22.** Choir influence networks carry leadership structure. Metric: **out-degree centralization (Gini of node out-degree) vs a density-matched Erdős-Rényi null** (0 = democratic, →1 = one leader). Status: **partially supported in real human choirs only** (Dagstuhl 3/5, ESMUC 2/3 pieces significant; ChoralSynth 2/20 ≈ chance; mean Gini 0.154 vs null 0.138 per `_h2_centralization.csv`, a modest effect). The original "topology shifts democratic→leader-dominated *as latency rises*" form is **data-blocked**: injecting a uniform delay into already-coordinated audio cannot create a behavioural leader, and fixed-lag Granger saturates under delay. Testing the latency form needs real low-vs-high-latency *live* recordings. Test: `scripts/h2_centralization_test.py`.
