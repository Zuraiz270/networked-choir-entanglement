...
def s2_recap(prs):
    ...
    _takeaway(
        slide, "This iteration: we found the latency signal, and learned exactly where it lives."
    )


def s3_headline(prs):
    slide = _content_slide(
        prs, 3, "Progress · last iteration", "We found the H1 latency signal (in attack timing)"
    )
    steps = [
        (
            "1 · Constant delay",
            "No E(t) effect. Our coupling is lag-tolerant; it absorbs a constant shift.",
        ),
        (
            "2 · Realistic jitter",
            "Still flat. 10s loudness envelopes are robust to tens-of-ms timing noise.",
        ),
        (
            "3 · Onset synchrony",
            "Zero-lag attack timing, the quantity latency breaks. The signal appears.",
        ),
    ]
    y = Inches(1.5)
    for t, b in steps:
    ...
    _takeaway(
        slide,
        "Latency degrades attack timing, not loudness coupling. The dissociation is the finding.",
    )
...
def s5_replication(prs):
    ...
    _stat_card(
        slide,
        MARGIN + 2 * (rail + Inches(0.2)),
        Inches(1.6),
        rail,
        Inches(1.5),
        "−76%",
        "ChoralSynth (synthetic, 20)",
    )
    _text(
        slide,
        MARGIN,
    ...
    _takeaway(
        slide, "The effect holds across real and synthetic choirs, not a single-dataset artifact."
    )
...
def s7_next(prs):
    ...
    rows = [
        ...
        (
            "WP2 / H3",
            "First visual-onset V(t): pair pose onsets with audio onsets on the 18 usable Tier-1 videos",
        ),
    ]
    y = Inches(1.6)
...
def s8_retro(prs):
    slide = _content_slide(
        prs, 8, "Retrospective · open problems", "Retrospective and open problems"
    )
    col = Inches(3.95)
    _card(
        slide,
        ...
        [
            ...
            "",
            "Every dataset claim traces to a verified file.",
        ],
        size=12.5,
    )
    _card(
        slide,
        MARGIN + col + Inches(0.18),
        Inches(1.5),
        col,
        Inches(3.4),
        "What we fixed",
        [
            ...
            "",
            "Curation now filters on singer visibility, not just NMP regime.",
        ],
        size=12.5,
    )
    _card(
        slide,
        MARGIN + 2 * (col + Inches(0.18)),
        Inches(1.5),
        col,
        Inches(3.4),
        "Open problems",
        [
            ...
            "",
            "H3 visual still data-blocked; H2 topology not cleanly measured yet.",
        ],
        size=12.5,
    )
    _text(
