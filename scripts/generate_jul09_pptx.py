...
def s2_goals_plan(prs: Presentation) -> None:
    ...
    rows = [
        ...
        (
            "H3 and plan",
            "Test visual/body signals when data exists; prepare the report and final dashboard demo.",
        ),
    ]
    y = Inches(2.25)
    ...
    _takeaway(
        slide,
        "Today we connect the project goal to the latest results and the final-presentation plan.",
    )
...
def s3_iteration_progress(prs: Presentation) -> None:
    ...
    for title, body in rows:
        ...
        y += Inches(0.82)
    _takeaway(
        slide, "The work moved from finding results to preparing the final presentation and report."
    )
...
def s4_report(prs: Presentation) -> None:
    ...
    _stat_card(
        slide, Inches(11.5), Inches(1.6), Inches(1.3), Inches(1.25), "H3", "Open", number_size=25
    )
    _text(
        slide,
        Inches(7.0),
...
def s6_h2(prs: Presentation) -> None:
    ...
    _takeaway(
        slide,
        "Leadership appears as a human coordination signal, not a synthetic-rendering artifact.",
    )
...
def s7_dashboard(prs: Presentation) -> None:
    ...
    _takeaway(
        slide,
        "The final demo can be clear and honest even though no current piece has all three signals.",
    )
...
def s8_remaining(prs: Presentation) -> None:
    ...
    rows = [
        ...
        (
            "Reproducibility",
            "Run final checks and regenerate headline figures from committed outputs.",
        ),
...
def s9_final_alignment(prs: Presentation) -> None:
    slide = _content_slide9(
        prs, 9, "Alignment", "What we need to align before the final presentation"
    )
    col = Inches(3.95)
    gap = Inches(0.18)
    _card(
