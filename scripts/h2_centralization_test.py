...
the same node and edge count (1000 draws), two-sided empirical p.
...
def null_gini_distribution(n_nodes: int, n_edges: int, rng: np.random.Generator) -> np.ndarray:
    ...
    for k in range(N_NULL):
        ...
        outdeg = np.bincount(sources[chosen], minlength=n_nodes)
        out[k] = gini(outdeg)
    return out
...
def run() -> None:
    ...
    for _, r in clean.iterrows():
        ...
        # two-sided empirical p: how often |null - mean| >= |obs - mean|
        p_more = float((null >= obs).mean())
        rows.append({
            "dataset": r["dataset"], "piece": r["piece"], "unit": r["unit"],
            "n_singers": n, "n_edges": m, "density": round(m / (n * (n - 1)), 3),
    ...
    print(f"Mean observed out-degree Gini: {out['obs_gini_outdeg'].mean():.3f} "
          f"(null mean {out['null_gini_mean'].mean():.3f})")
    print(f"Pieces significantly MORE centralized than random (p<0.05): "
          f"{len(sig)} / {len(out)}")
    print(f"Mean density: {out['density'].mean():.2f}")
    for ds in ["dagstuhl", "esmuc", "choralsynth"]:
        d = out[out.dataset == ds]
        if len(d):
            print(f"  {ds}: obs Gini {d['obs_gini_outdeg'].mean():.3f} vs null "
