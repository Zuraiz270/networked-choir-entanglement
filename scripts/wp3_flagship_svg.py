...
def render(piece: str, method: str) -> Path:
    ...
    nx.draw_networkx_labels(
        g, pos, labels={n: str(n) for n in g.nodes},
        font_size=12, font_color="white", font_weight="bold", ax=ax,
    )
    if g.number_of_edges():
        ...
        nx.draw_networkx_edges(
            g, pos, edgelist=[(u, v) for u, v, _ in edges], width=widths,
            edge_color="#555555", alpha=0.7, arrows=True, arrowsize=20,
            connectionstyle="arc3,rad=0.12", node_size=2200, ax=ax,
        )

    present = sorted({str(n)[0] for n in g.nodes})
    handles = [
        plt.Line2D([0], [0], marker="o", linestyle="", markersize=11,
                   markerfacecolor=VOICE_COLOR[s], markeredgecolor="none",
                   label=VOICE_NAME.get(s, s))
        for s in present if s in VOICE_COLOR
    ]
